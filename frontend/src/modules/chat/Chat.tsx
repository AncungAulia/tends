...
export function Chat({ onClose, expanded, onToggleExpand }: ChatProps) {
          ...
          >
            ...
            {isUser ? (
              address ? (
                ...
                <div className="mt-0.5 h-7 w-7 shrink-0 rounded-full bg-tip dark:bg-white" />
              )
            ) : (
              <div className="mt-0.5 h-7 w-7 shrink-0 rounded-full bg-brand" />
            )}
            ...
            {streamingEmpty ? (
              ...
              <div
                className={cn(
                  ...
                  isUser
                    ? "whitespace-pre-wrap rounded-tr-none bg-tip text-white dark:bg-white dark:text-ink"
                    : "rounded-tl-none bg-panel text-ink dark:bg-white/5 dark:text-white",
                )}
              >
  ...
  return (
    <div
      className={cn(
        "z-40 flex flex-col overflow-hidden bg-card",
        expanded
          ? // Full content area (right of the sidebar), no shadow so it never
            // bleeds over the sidebar.
            "fixed inset-y-0 right-0 left-0 border-l border-edge md:left-60"
          : "fixed bottom-4 right-4 h-[28rem] w-[min(95vw,22rem)] rounded-2xl border border-edge shadow-2xl max-md:bottom-20",
      )}
    >
      <header className="flex items-center justify-between border-b border-edge px-4 py-3">
        ...
        <div className="flex items-center gap-3 text-dim">
          {expanded && !empty && (
            <button
              onClick={reset}
              className="inline-flex items-center gap-1.5 rounded-lg border border-edge px-2.5 py-1 font-mono text-[0.65rem] uppercase tracking-[0.04em] transition-colors hover:border-ink hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40 dark:hover:text-white"
            >
              <Plus size={13} />
              New chat
            </button>
          ...
          <button
            ...
            className="hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40 dark:hover:text-white"
          >
        ...
        {empty ? (
          expanded ? (
            ...
            <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
              <h2 className="max-w-xl font-sans text-3xl font-bold tracking-[-0.02em] text-ink sm:text-4xl">
              ...
              <div className="mt-7 flex flex-wrap justify-center gap-2">
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
                    onClick={() => send(s)}
                    className="rounded-full border border-edge px-4 py-2 text-sm text-ink transition-colors hover:border-brand hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40 dark:text-white/70"
                  >
                    {s}
                  </button>
                ))}
            ...
            <p className="p-4 font-sans text-sm text-dim">
      ...
      >
        <div className={cn("flex items-center gap-2", expanded && "mx-auto w-full max-w-2xl")}>
          <input
            ...
            className={cn(
              "flex-1 rounded-xl border border-edge bg-card outline-none focus:border-brand disabled:opacity-50 dark:bg-tip dark:text-white",
          ...
          <button
            ...
            aria-label="Send"
            className={cn(
              "flex items-center justify-center rounded-xl bg-brand text-white transition-opacity disabled:opacity-40",
