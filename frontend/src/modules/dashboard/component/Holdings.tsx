...
export function Holdings(_: HoldingsProps) {
  ...
  const centerPct =
    hover === null || hover >= shown.length
      ? 0
      : shown.slice(0, hover).reduce((s, h) => s + h.allocationPct, 0) +
        (shown[hover]?.allocationPct ?? 0) / 2;
              ...
              {hover !== null && hover < shown.length && (
                ...
                >
                  ...
                  <p className="text-[10px] text-white/55">
                    {shown[hover].allocationPct.toFixed(1)}% ·{" "}
                    ${Number(shown[hover].valueUsd).toLocaleString("en-US", { maximumFractionDigits: 2 })}
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-dim">
                    {Number(h.balance).toLocaleString("en-US", { maximumFractionDigits: 4 })}
                  </p>
                </div>
                ...
                <div className="w-20 text-right">
                  {Number(h.valueUsd) > 0 ? (
                    <p className="flex items-center justify-end text-sm font-semibold text-ink">
                      <span>$</span>
                      <SlidingNumber number={Number(h.valueUsd)} decimalPlaces={2} />
                    </p>
                  ) : (
